...
description='This script runs the pilot for the SemEval2016 task-2 interpretable STS')
parser.add_argument('--sent1', help='File containing first sentences in input sentence pairs', required=True)
parser.add_argument('--sent2', help='File containing second sentences in input sentence pairs', required=True)
parser.add_argument('--chunked', help='Whether sentences are chunked or not (True / False)', required=True)
    ...
    if sinonim == True:
        ...
        for synset1 in wn.synsets(word1):
            for synset2 in wn.synsets(word2):
                ...
                if synset1.path_similarity(synset2) <= 0.1:
                    label = ' // NOALI // NIL //'
                elif ((synset1.path_similarity(synset2) >= 0.7) and (synset1.path_similarity(synset2) <= 1)):
                    label = ' // REL // 4 // '
                elif ((synset1.path_similarity(synset2) >= 0.15) and (synset1.path_similarity(synset2) <=0.6)):
                        label = ' // REL // 3 // '
...
def writeMaximunRelatedChunkIds(MaximunRelatedChunkIds, sentence1TokensChunk, sentence2TokensChunk):
    for bestPairCids in MaximunRelatedChunkIds:
        ...
        a=[x.encode('UTF8') for x in s1CidTokensText]
        b= [x.encode('UTF8') for x in s2CidTokensText]
        ketemu = False
    ...
    for S1NotLinkedPairCids in Sentence1_NotLinked_chunks:
        ...
        for S2NotLinkedPairCids in Sentence2_NotLinked_chunks:
            ...
            try :
                ...
                sys.stdout.write(str(s1CidTokens).replace("[", "").replace("]", "").replace(",", ""))
                #sys.stdout.write(str(" ").replace("[", "").replace("]", "").replace(",", ""))
            sys.stdout.write(' <==> ')
            sys.stdout.write(str(s2CidTokens).replace("[", "").replace("]", "").replace(",", ""))
            try :
...
for pair_id in range(len(sent1_lines)):
	...
	#print" sentence2chunk",sentence1Chunks
